...
export default function TabelaFornecedores({ fornecedores = [], onEdit, onDelete, onPrint, isLoading, showConfigColunas, setShowConfigColunas }) {
  ...
  const renderCell = (item, colId) => {
    ...
    if (colId === "nome") return <span className={`uppercase font-semibold ${item.ativo === false ? 'text-[var(--mg-text-3)] line-through' : ''}`}>{item.nome || "-"}</span>;
    if (colId === "cpf_cnpj") return <span className="font-mono">{item.cpf || item.cnpj || "-"}</span>;
    if (colId === "telefone") return item.telefone || "-";
    if (colId === "email") return item.email || "-";
    if (colId === "cidade") return item.cidade || "-";
    if (colId === "estado") return item.estado || "-";
    if (colId === "ativo") return item.ativo !== false ? <Badge className="text-[10px] bg-[var(--mg-green-light)] text-[var(--mg-green)]">Ativo</Badge> : <Badge className="text-[10px] bg-[var(--mg-gray-fill)] text-[var(--mg-text-3)]">Inativo</Badge>;
    return "-";
  };

  const renderFilterControl = (colunaId) => {
    const btnCls = `h-3 w-3 min-w-3 p-0 ${hasActiveFilter(colunaId) ? "text-[var(--mg-accent)]" : "text-[var(--mg-text-3)] hover:text-[var(--mg-text-1)]"}`;
      ...
      <Popover open={menuFiltroAberto === colunaId} onOpenChange={(open) => { setMenuFiltroAberto(open ? colunaId : null); setBuscaFiltroMenu(""); setFiltroTemp(open ? { colunaId, valores: [...getValoresFiltro(colunaId)] } : { colunaId: null, valores: [] }); }}>
        ...
        <PopoverContent align="end" side="bottom" sideOffset={4} className="w-[310px] p-0 z-[9999]">
          <div className="p-1 space-y-0.5 border-b">
            <button type="button" className="flex items-center w-full px-2 h-8 text-xs hover:bg-[var(--mg-gray-fill)] rounded" onClick={() => { handleSort(colunaId); setMenuFiltroAberto(null); }}><ArrowDownAZ className="w-4 h-4 mr-2" /> Menor → Maior</button>
            <button type="button" className="flex items-center w-full px-2 h-8 text-xs hover:bg-[var(--mg-gray-fill)] rounded" onClick={() => { setSortConfig({ key: colunaId, direction: "desc" }); setMenuFiltroAberto(null); }}><ArrowUpZA className="w-4 h-4 mr-2" /> Maior → Menor</button>
            <button type="button" className={`flex items-center w-full px-2 h-8 text-xs rounded ${hasActiveFilter(colunaId) ? 'hover:bg-[var(--mg-gray-fill)] text-[var(--mg-text-1)]' : 'text-[var(--mg-text-3)] cursor-not-allowed'}`} disabled={!hasActiveFilter(colunaId)} onClick={() => { setValoresFiltro(colunaId, []); setMenuFiltroAberto(null); }}><X className="w-4 h-4 mr-2" /> Limpar Filtro</button>
          </div>
          <div className="p-2 space-y-2">
            ...
            <div className="border border-[var(--mg-border)] rounded-sm max-h-64 overflow-y-auto p-1 bg-white">
              <label className="flex h-8 items-center gap-2 px-2 py-0 text-xs text-[var(--mg-text-1)] border-b border-[var(--mg-border)]">
              ...
              {filteredOpts.map(option => (
                <label key={option} className="flex h-6 items-center gap-2 px-2 py-0 text-xs text-[var(--mg-text-1)] hover:bg-[var(--mg-gray-fill)]">
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              ...
              <Button size="sm" className="h-8 text-xs bg-[var(--mg-accent)] hover:bg-[var(--mg-accent-dark)] text-white" onClick={() => { setValoresFiltro(colunaId, filtroTemp.valores); setMenuFiltroAberto(null); }}>OK</Button>
            </div>
          </div>
        </PopoverContent>
      </Popover>
  ...
  return (
    <div className="space-y-1 overflow-hidden">
      <div className="flex justify-between items-center px-1 gap-2 flex-wrap">
        <div className="text-xs text-[var(--mg-text-3)]">{sortedFornecedores.length} de {fornecedores.length} registros</div>
        ...
        <CardContent className="p-0 overflow-hidden">
          <div ref={scrollContainerRef} className="relative w-full overflow-auto max-h-[calc(100dvh-240px)] md:max-h-[calc(100dvh-150px)]" style={{ overscrollBehavior: 'none', WebkitOverflowScrolling: 'touch' }}>
            <Table ref={tableRef} className={`w-full ${isMobile ? "min-w-[720px]" : "min-w-[1000px]"} border-separate border-spacing-0 table-fixed`}>
                ...
                <TableRow className="sticky top-0 z-40 bg-white">
                  {colunasOrdenadas.map(coluna => {
                    ...
                    if (coluna.id === "selecao") return (
                      <TableHead key="selecao" style={{ width: 25, minWidth: 25, maxWidth: 25 }} className="sticky top-0 z-40 h-7 p-0 bg-[var(--mg-gray-fill)] text-center align-middle border-r border-b border-[var(--mg-border)]">
                        <div className="flex items-center justify-center w-full h-full"><Checkbox checked={selectedItems.length === sortedFornecedores.length && sortedFornecedores.length > 0} onCheckedChange={toggleSelectAll} className="h-4 w-4 rounded-full border-2 border-[var(--mg-text-3)] data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground" /></div>
                      </TableHead>
                    );
                    if (coluna.id === "acoes") return <TableHead key="acoes" style={{ width: 25, minWidth: 25, maxWidth: 25 }} className="sticky top-0 z-40 h-7 p-0 bg-[var(--mg-gray-fill)] text-center align-middle border-r border-b border-[var(--mg-border)]" />;
                    const fc = renderFilterControl(coluna.id);
                    return (
                      <TableHead key={coluna.id} style={{ width, minWidth: width, maxWidth: width }} className="sticky top-0 z-40 relative align-middle text-[var(--mg-text-1)] px-2 pr-7 text-xs font-medium text-center border-r border-b border-[var(--mg-border)] bg-[var(--mg-gray-fill)] whitespace-nowrap h-7">
                        ...
                        {fc && (
                          <div className="absolute right-1 top-1/2 -translate-y-1/2 z-50 flex items-center gap-1" onClick={e => e.stopPropagation()}>
                            {fc}
                            <button type="button" className={`h-4 w-4 flex items-center justify-center rounded ${isResizing ? 'text-[var(--mg-accent)] bg-[var(--mg-accent-light-bg)]' : 'text-[var(--mg-text-3)] hover:text-[var(--mg-text-1)]'}`} onClick={e => { e.stopPropagation(); toggleResizeMode(coluna.id); }}><GripVertical className="w-2.5 h-2.5" /></button>
                          </div>
                        )}
                        {isResizing && (
              ...
              <TableBody>
                {isLoading ? (
                  <TableRow><TableCell colSpan={colunasOrdenadas.length} className="text-center py-8 text-xs text-[var(--mg-text-3)] border border-[var(--mg-border)]">Carregando...</TableCell></TableRow>
                ) : sortedFornecedores.length === 0 ? (
                  <TableRow><TableCell colSpan={colunasOrdenadas.length} className="text-center py-8 text-xs text-[var(--mg-text-3)] border border-[var(--mg-border)]">Nenhum fornecedor/cliente encontrado</TableCell></TableRow>
                ) : sortedFornecedores.map(item => (
                  <TableRow key={item.id} className="transition-colors border-b hover:bg-[var(--mg-accent-light-bg)]" onDoubleClick={() => onEdit(item)} onTouchEnd={(event) => handleRowTouch(item, event)}>
                    {colunasOrdenadas.map(coluna => {
                      const width = columnWidths[coluna.id] || coluna.width || 160;
                      if (coluna.id === "selecao") return (
                        <TableCell key={`${item.id}-sel`} style={{ width: 25 }} className="p-0 text-center align-middle h-7 border-r border-b border-[var(--mg-border)]" onClick={e => e.stopPropagation()}>
                          <div className="flex items-center justify-center w-full h-full"><Checkbox checked={selectedItems.includes(item.id)} onCheckedChange={(checked) => setSelectedItems(prev => checked ? [...prev, item.id] : prev.filter(id => id !== item.id))} className="rounded-full h-4 w-4 border-2 border-[var(--mg-text-3)] data-[state=checked]:bg-primary" /></div>
                        </TableCell>
                      );
                      if (coluna.id === "acoes") return (
                        <TableCell key={`${item.id}-act`} style={{ width: 25 }} className="p-0 text-center align-middle h-7 border-r border-b border-[var(--mg-border)]" onClick={e => e.stopPropagation()}>
                          <div className="flex items-center justify-center w-full h-full">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="start">
                                ...
                                <DropdownMenuSeparator />
                                <DropdownMenuItem onClick={() => setDeleteConfirm(item)} className="text-xs text-[var(--mg-red)]">Excluir</DropdownMenuItem>
                              </DropdownMenuContent>
                            </DropdownMenu>
                          </div>
                        </TableCell>
                      ...
                      return <TableCell key={`${item.id}-${coluna.id}`} style={{ width, minWidth: width, maxWidth: width }} className={`px-2 py-1 text-[var(--mg-text-1)] text-xs align-middle border-r border-b border-[var(--mg-border)] whitespace-normal break-words ${align}`}>{renderCell(item, coluna.id)}</TableCell>;
                    })}
                  </TableRow>
                ))}
              </TableBody>
